...
最後に Codex 側で実行しやすい JSON を必ず `json` コードブロックで出してください。
...
- `shape_balance` では `right_dom_ge2/4/6` と、`opposite_sign` に右側優位を重ねた `opp_right_dom_ge2/4/6` を使えます
...
7. 現行 core 未実装の label が必要なら、Codex 宛の実装仕様書を書いてください
...
新規 label が不要な場合は、`新規 label 実装仕様書` は「なし」としてください。
新規 label が必要な場合は、Codex 宛の仕様書を `markdown` コードブロックで出してください。
Codex 実行用 JSON は必ず `json` コードブロックで出してください。
