...
from qualify.common.e001 import run_e001
from qualify.common.params import load_e001_params


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run qualify E001 with canonical backtest_1m engine.")
    parser.add_argument("--params-file", required=True, help="Path to ChatGPT-produced JSON params.")
    parser.add_argument("--dataset-dir", default="dataset")
    parser.add_argument("--out-dir", default="qualify/out/E001/latest")
...
def main() -> None:
    args = parse_args()
    params = load_e001_params(args.params_file)
    print(f"[START] E001 params={args.params_file} out={args.out_dir}")
    print(f"[YEARS] {' '.join(str(year) for year in args.years)}")
    print(f"[COMPARISONS] {len(params.comparison_labels)} labels")
    result = run_e001(
        params=params,
