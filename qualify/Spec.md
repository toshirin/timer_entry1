...
`qualify` では 1 slot に複数 candidate version を持てるよう、`params` と `out` は `slot_id` の下に `vN` ディレクトリを置く。

- params
  - `qualify/params/{slot_id}/v1/e001.json`
  - `qualify/params/{slot_id}/v2/e001.json`
- out
  - `qualify/out/{slot_id}/v1/E004/latest/...`
  - `qualify/out/{slot_id}/v2/E004/latest/...`

`results` と `runtime/config` は file name の `..._v1` で version を表す。

現役ではない候補や旧成果物は `archived` 配下に置く。

- archived params
  - `qualify/params/archived/{slot_id}/{version_id}/e001.json`
- archived out
  - `qualify/out/archived/{slot_id}/{version_id}/E004/latest/...`
- archived results
  - `qualify/results/archived/{slot_id}/{version_id}/{result_id}.json`

`version_id` は active と同様に candidate version を表す。既存の旧候補アーカイブは `v0` を使う。
...
- params
  ...
  - CLI 引数では sweep / maintenance margin 条件を受けない
...
- E007
  ...
  - params では `target_maintenance_margin_candidates` を使う
  - summary には少なくとも `target_maintenance_margin_pct`, `annualized_pips`, `cagr`, `trade_rate`, `win_rate`, `max_dd_pct`, `min_maintenance_margin_pct`, `maintenance_below_130_count`, `maintenance_below_100_count`, `stop_triggered`, `final_equity_jpy`, `total_return_pct`, `pips_year_rate_pct_at_150usd` を出す
  - E007 summary の `min_maintenance_margin_pct` と below count は、entry 直後ではなく即時 SL 到達時の想定維持率で評価する
  - `maintenance_below_100_count > 0` は一発NGとする
  - `stop_triggered` または `maintenance_below_130_count > 0` は、その維持率が強すぎるシグナルとして一段上の候補を確認する
  - 採用判断は CAGR 最大ではなく、安全条件を満たした最初の維持率候補を優先する
...
runtime promotion は `qualify/params/{slot_id}/{version_id}/*.json` ではなく、この最終昇格結果 JSON を入力の主経路にする。
