...
def _comparison_trade_frame(
    *,
    ...
    for trade in result.trades:  # type: ignore[attr-defined]
        ...
        row["forced_exit_clock_local"] = forced_exit_clock_local
...
def run_e003(
    *,
    ...
    for forced_exit_clock_local in params.forced_exit_values:
        ...
        all_trade_frames.append(
            _comparison_trade_frame(
                comparison_label=comparison_label,
                filter_label=filter_label,
                tp_pips=params.baseline.tp_pips,
                sl_pips=params.baseline.sl_pips,
                forced_exit_clock_local=forced_exit_clock_local,
                result=result,
            )
        )
        ...
        sanity_row["tp_pips"] = float(params.baseline.tp_pips)
        sanity_row["sl_pips"] = float(params.baseline.sl_pips)
        sanity_row["forced_exit_clock_local"] = forced_exit_clock_local
    ...
    summary_df = build_e003_summary(summary_rows)
    ...
    metadata = {
        ...
        "tp_pips": params.baseline.tp_pips,
        "sl_pips": params.baseline.sl_pips,
        "forced_exit_values": list(params.forced_exit_values),
