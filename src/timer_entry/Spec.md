...
qualify では、必要に応じて `right_strength_balance` の threshold sweep を追加してよい。
初版では以下を許容する。
...
`StrategySetting.to_runtime_config()` は `exclude_windows` を `execution_spec_json.exclude_windows` へ変換する。
ただし `StrategySetting.execution_spec` に同名キーが既に存在する場合は、明示された `execution_spec` 側を優先し、`exclude_windows` フィールドでは上書きしない。
runtime promotion / 設定投入時は、`lon12` など除外 window を必要とする setting の `execution_spec_json.exclude_windows` に期待値が含まれていることを確認する。
...
- `fixed_slippage_pips`
- `entry_delay_seconds`
- `target_maintenance_margin_pct`
- `risk_fraction`（legacy / deprecated）
