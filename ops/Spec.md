...
8. 未処理の `skipped_kill_switch` fact があれば、unit level の即時 1 段降格 path を実行する
9. summary を更新する
10. 突合異常や処理結果をログ出力する
...
### 15.1 Unit Level との接続

通常の unit level 昇降格は月次 batch で扱う。
kill-switch や将来の異常検知による即時降格は、月次 batch とは別の入口から同じ unit level apply path を呼び出す。

異常検知系はまだ初版では実装しない。
ただし、将来 `劣化検知バッチ` や runtime anomaly handler を追加した場合は、以下の形で unit level 側へ接続する。

- `source` に異常種別を入れる
- `decision_reason` に具体的な検知理由を入れる
- `next_level = max(current_level - 1, 0)` の即時 1 段降格を適用する
- `unit_level_decision_log` に履歴を保存する
- runtime の `setting_config` に反映する

適用順序は unit level の共通ルールに合わせる。
先に `unit_level_decision_log` へ pending 記録を作り、runtime の `setting_config` を更新し、最後にログを `applied = true` にする。
再試行時に pending ログが残っている場合は、現在 level から降格先を再計算せず、ログに保存済みの `next_level` を使う。

これにより、月次の通常昇降格、kill-switch、異常検知の経路は分けつつ、level 更新と監査ログの形式は共通化する。
