...
初版では、kill-switch による降格は ops の `daily_transaction_import` 後に `skipped_kill_switch` fact を検知して適用する。
そのため実運用上は、kill-switch 発生後の次回日次 import で 1 段降格される。
将来、より即時性が必要になった場合は、同じ unit level apply path を別 Lambda から呼び出す。
...
同じ `setting_id` と `decision_month` に対して、通常の月次判定を重複適用してはならない。

実装では、先に `unit_level_decision_log` へ `applied = false` の pending 記録を作成し、その後に runtime の `setting_config` を更新し、最後に同じログを `applied = true` に更新する。
途中で失敗して Lambda が再試行された場合は、現在の level から再計算せず、pending ログに保存済みの `next_level` を再利用する。
これにより、DynamoDB 更新後に Aurora ログ更新だけ失敗しても、再試行で二重昇格・二重降格しない。
